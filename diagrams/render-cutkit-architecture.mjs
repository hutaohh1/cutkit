...
out.push(`<text x="60" y="108" fill="#94a3b8" font-size="21">面向 Codex 的本地优先剪辑引擎 · 五层架构 · 制作闭环 · 创作能力注册表</text>`);

panel(60, 140, 1680, 180, "一、目标与硬约束");
card(100, 198, 760, 92, "A1 本地 CLI 垂直切片", ["P0 编辑协议 · P1 真实 FFmpeg/ffprobe 媒体执行"], "goal");
card(920, 198, 760, 92, "产品硬约束", ["timeline.json 唯一执行真相 · 原片只读 · 不默认上传"], "goal");
...
panel(60, 2340, 1680, 360, "六、创作能力注册与渲染路径");
card(100, 2410, 380, 220, "概念拆分", ["Effect / Transition / Motion", "Overlay / Compositing / Audio"], "flow");
card(520, 2410, 380, 220, "特效与转场来源", ["FFmpeg 媒体原语", "HTML / SVG / Canvas", "Shader / 素材包"], "flow");
card(940, 2410, 380, 220, "字体 / 字幕 / 音频", ["事件 → ASS → libass", "dialogue / vo / music / sfx"], "flow");
card(1360, 2410, 340, 220, "Render 三路径", ["原生媒体处理", "局部动画渲染", "局部复杂合成"], "flow");

panel(60, 2740, 1680, 180, "七、后续优先级");
card(110, 2805, 720, 85, "P1 收尾", ["xfade / 多层 compositor · Remotion / HyperFrames · golden fixtures"], "p1");
card(970, 2805, 720, 85, "P2 后续", ["C2PA / 权利与同意 · OTIO / EDL / FCPXML · 外部编辑器交换"], "p2");
...
out.push(`</svg>`);
const svg = out.join("\n");
await writeFile("D:/项目3/cutkit-v2-architecture.svg", svg, "utf8");
const png = new Resvg(svg, {
  fitTo: { mode: "width", value: 3600 },
  background: "#0b0f14",
  font: { loadSystemFonts: true, defaultFontFamily: "Segoe UI, Microsoft YaHei, sans-serif" }
}).render().asPng();
await writeFile("D:/项目3/cutkit-v2-architecture.png", png);
console.log(JSON.stringify({ svg: "D:/项目3/cutkit-v2-architecture.svg", png: "D:/项目3/cutkit-v2-architecture.png", width: 3600, height: 5960 }));
